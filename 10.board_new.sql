...
insert into address(country,city,street,author_id) values('USA','NEWYORK','asdf',2);
...
select p.title,p.contents,a.name from post p inner join author_post_list apl inner join author a on p.id=apl.post_id and a.id=apl.author_id;
