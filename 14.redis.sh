# window에서는 redis가 직접설치가안됨 -> 도커를 통한 redis설치
docker run --name my-redis-container -d -p 6379:6379

# redis 접속 명령어
redis-cli

# docker에 설치 된 redis 접속 명령어
docker ecec -it 컨테이너id redis-cli

# redis는  0~15까지의 db로 구성(default 0번)
# db번호 선택
select db번호
...
# 이미 존재하는 key을 set 하면 덮어쓰기
set user:email:1 hong2@naver.com
# key 값이 이미 존재하면 pass시키고 없을때만 set 하기 위해선 nx 옵션 사용
set user:email:1 hong3@naver.com nx
# 만료시간(ttl) 설정은 ex옵션 사용 (초 단위)
set user:email:1 hong3@naver.com ex 30
# 특정 key값 삭제
del 키값
# 현재 db내 모든 key값 삭제
flushdb
...
lrange students 0 -1 # 전체조회
lrange students 0 0 #0번째값만 조회
lrange students -1 -1 #마지막값만 조회
lrange students -2 -1 # 뒤에서 2번째부터 마지막 조회
# list 값 꺼내기(꺼내면서 삭제)
rpop students # 오른쪽 끝 값 꺼내기
lpop students # 왼쪽 끝 값 꺼내기
# A리스트에서 rpop 하여 b리스트에 lpush : 잘 안쓰임 << 곧 사라짐
rpoplpush A리스트 B리스트
rpoplpush stuents students
# list의 데이터 개수 조회
llen students
# expire, ttl 문법 모두 사용가능

# redis List자료구조 실전 활용
# 사례1 : 최근 조회 한 상품 목록
...
# zset 도 set 이므로 같은 상품을 add할 경우에 중복이 제거되고,score(시간)값만 업데이트
...
# zset 조회: zrange(score기준 오름차순 정력), zrevrange(내림차순정렬)
...
# redis
# 1) pub/sub : 메시지 저장 xX && 전파 -> 성능빠름. 메시지 유실가능성.
